...
Deno.test("isUsableWord accepts plain lowercase words", () => {
  assert(isUsableWord("content"));
  assert(isUsableWord("bright"));
});

Deno.test("isUsableWord rejects phrases, hyphens, and odd lengths", () => {
...
Deno.test("frequencyFromTags parses the f: metadata tag", () => {
...
Deno.test("difficultyFromFrequency maps frequency onto the 1-5 scale", () => {
...
Deno.test("pickDistractors returns the requested count drawn from the pool", () => {
...
Deno.test("pickDistractors is deterministic with a seeded rng", () => {
...
Deno.test("pickDistractors caps at the pool size", () => {
