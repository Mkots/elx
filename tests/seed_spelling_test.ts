...
Deno.test("hasSingleGap accepts a sentence with exactly one gap", () => {
  assert(hasSingleGap(`The cat sat on the ___ mat.`));
});

Deno.test("hasSingleGap rejects sentences with no gap or many gaps", () => {
  assert(!hasSingleGap("No gap here at all."), "missing gap");
  assert(!hasSingleGap(`A ___ and another ${GAP} gap.`), "two gaps");
});

Deno.test("spelling seeds contain no duplicate words", () => {
...
Deno.test("spelling seed words are usable single tokens", () => {
...
Deno.test("spelling seeds have difficulty in range 1-5", () => {
  for (const seed of spellingSeeds) {
    assert(
...
Deno.test("every spelling sentence has exactly one gap", () => {
  for (const seed of spellingSeeds) {
    assert(
...
Deno.test("spelling sentences never contain the answer word", () => {
  for (const seed of spellingSeeds) {
